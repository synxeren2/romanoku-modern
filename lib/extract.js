...
// ---------- PDF ----------
async function extractPdf(filePathOrBuffer) {
...
// ---------- Unified entry ----------
async function extractBook(filePathOrBuffer, declaredKind) {
  let kind = declaredKind;
  if (!declaredKind && typeof filePathOrBuffer === 'string') {
    kind = (path.extname(filePathOrBuffer) || '').toLowerCase().replace('.', '');
  }
  let result;
  if (kind === 'pdf') {
    result = await extractPdf(filePathOrBuffer);
  } else if (kind === 'epub') {
    result = extractEpub(filePathOrBuffer);
  } else {
